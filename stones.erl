...
main(_Args) ->
    ...
    io:format("~p possible combinations generated.~n", [length(StoneWeights)]),
...
%% stone_configs() -> [ {St1,St2,St3,St4} || St1 <- lists:seq(1,37), 
%%                                           St2 <- lists:seq(1,37), 
%%                                           St3 <- lists:seq(1,37), 
%%                                           St4 <- lists:seq(1,37), 
%%                                           (St1 + St2 + St3 + St4) =:= 40 ].
%% %
%% % We need to make sure that at least one of the possible equations for 
%% % X (+/-) S1 (+/-) S2 (+/-) S3 (+/-) S4 = 0
%% % exists for each of the X ranging from 1 to 40
%% %
%% sloves( {St1,St2,St3,St4} ) ->
%%     [ {St1,St2,St3,St4} || X    <- lists:seq(1,40), 
%%                            Sgn1 <- [1, -1],
%%                            Sgn2 <- [1, -1],
%%                            Sgn3 <- [1, -1],
%%                            Sgn4 <- [1, -1],
%%                            X + Sgn1 * St1 + Sgn2 * St2 + Sgn3 * St3 + Sgn4 * St4 =:= 0 ].
